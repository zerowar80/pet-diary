...
그 외에도 여러 사진을 한 일기로 묶기, 30초 이내 짧은 동영상 일기, AI 얼굴 자동 인식, 월간 하이라이트, 노래 가사 생성, 로그인 없이 보기만 가능한 게스트 모드, 책장 넘기듯 보는 일기 화면까지 지원합니다.
...
- **동영상 일기**: 업로드 화면에서 "짧은 동영상"을 선택하면 30초 이내(40MB 이내) 영상을 일기로 올릴 수 있습니다. **Gemini**를 고르면 영상을 통째로(소리 포함) 이해해서 일기를 쓰고, **Claude/ChatGPT**는 아직 영상을 직접 분석하지 못해서 영상의 대표 장면 한 컷을 자동으로 뽑아 사진처럼 분석합니다. 앨범에서는 재생 버튼이 있는 동영상으로 그대로 보입니다. (동영상 처리에는 서버에 `ffmpeg`가 필요하며, 설치 스크립트에 이미 포함되어 있습니다.)
